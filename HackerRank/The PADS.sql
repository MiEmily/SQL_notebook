...
group by x: put all those same values for x in the same row.All column names from the SELECT clause should either 
appear in the GROUP BY clause or be used in the aggregate functions. 

common aggregate function in sql:  ave(),max(),min(),sum(),count()
